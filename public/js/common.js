...
const init = (async _ =>{
	data = await fetch("/xml/festivalList.xml").then( v => v.text() );
	xml = [...$($.parseXML(data)).find("item")].map(  v => {
		return [...v.children].reduce( (acc,v,idx) => {
			let text = v.textContent;
			switch(v.tagName){
				case "images":
				text = [...v.children].map( v => v.textContent );
				break;
				case "sn":
				text = v.textContent.length == 1 ? "00"+v.textContent : "0"+v.textContent;
				break;
			};
			acc[v.tagName] = text;
			return acc;
